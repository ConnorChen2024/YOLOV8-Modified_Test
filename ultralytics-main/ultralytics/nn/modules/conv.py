...
class my_channel_attention(nn.Module):
    def __init__(self, channle):
        ...
        self.fc = nn.Sequential(
            nn.Conv2d(channle, channle//ratio, kernel_size=1, bias=False),
            nn.ReLU(),
            nn.Conv2d(channle//ratio, channle, kernel_size=1, bias=False),
            # nn.Conv2d(channle, channle // ratio),
            # nn.ReLU(),
            # nn.Conv2d(channle // ratio, channle),
        )
    ...
    def forward(self, x):
        ...
        max_pool_out = self.max_pool(x)  # [B, C, 1, 1]
        avg_pool_out = self.avg_pool(x)  # [B, C, 1, 1]
...
class my_spacial_attention(nn.Module):
    def __init__(self, kernel_size=7):
        super(my_spacial_attention, self).__init__()
...
class my_CBAM(nn.Module):
    def __init__(self, c1, kernel_size=7):
        ...
        self.spacial_attention = my_spacial_attention(kernel_size)
    def forward(self, x):
